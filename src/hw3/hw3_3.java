...
public class hw3_3 {
	public static void lastNumber(int n) {
		int lastNumber[] = new int[49];
		for (int o = 1; o <= 48; o++) {
			if(o % 10 != n && o / 10 != n) {
				lastNumber[o] = o;
//				System.out.print(lastNumber[o] +" ");
			}
//			else {
//				lastNumber[o] = ' ';
//			}
		...
		// 2. Fisher-Yates Shuffle 洗牌法隨機打亂陣列
		 Random random = new Random();
		 	for (int i = nonZeroArr.length - 1; i > 0; i--) {
	            int j = random.nextInt(i + 1); // 隨機交換 i 和 j
	            int temp = nonZeroArr[i];
	            nonZeroArr[i] = nonZeroArr[j];
	            nonZeroArr[j] = temp;
	        }
	        // 3. 取前 6 個數字
	        int[] selectedNumbers = Arrays.copyOfRange(nonZeroArr, 0, 6);
